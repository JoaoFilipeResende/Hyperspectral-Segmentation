...
RESULTS_FILE = "vis_knn_y_predict_3K.npy"
OUTPUT_METRICS_FILE = "vis_knn_3K_metrics.txt"
...
def process_prediction(y_predict, img_idx):
    ...
    for y in range(254):
        for x in range(510):
            img_offset = img_idx * 510 * 254
            px_idx = (y * 510 + x) + img_offset
...
if __name__ == '__main__':
    ...
    file_list = os.listdir(os.path.join(os.path.dirname(os.path.abspath(__file__)), "Dataset", "HyKo2", "vis"))
    file_list.sort()

    n_train_files = int(np.ceil(TRAIN_PERCENTAGE * len(file_list)))
    ...
    for idx in range(len(predict_files)):
        path = os.path.join(os.path.dirname(os.path.abspath(__file__)), "Dataset", "HyKo2", "vis", predict_files[idx])
        img = Hyperimage(path)
