...
class VisGenerator(tf.keras.utils.Sequence):
    ...
    def validate_batch(self, batch_idx):
        ...
        last_img_idx = int(start_px / (254 * 510))
        hyper_img = Hyperimage(os.path.join(os.path.dirname(os.path.abspath(__file__)), "Dataset", "HyKo2", "vis",
        ...
        if batch_idx == len(self) - 1:
            n_px_in_batch = len(self.file_list) * 254 * 510 - start_px

        for i in range(n_px_in_batch):
            img_idx = int((start_px + i) / (254 * 510))
            px_idx = start_px % (254 * 510)
    ...
    def __init__(self, batch_size, train_percent, validate_percent, predict_percent, gen_type, patch_size):
        ...
        self.length = np.ceil((len(self.file_list) * 254 * 510 / float(self.batch_size))).astype(np.int)
    ...
    def __len__(self):
        if (self.gen_type == "predict"):
            return np.ceil((len(self.file_list) * 254 * 510 / float(self.batch_size))).astype(np.int)
        else:
            return self.length
